...
_kafka_acls_args+=('--authorizer: Fully qualified class name of the authorizer, defaults to kafka. security.authorizer.AclAuthorizer.')
...
_kafka_acls_args+=('--operation: Operation that is being allowed or denied. Valid operation names are: Describe DescribeConfigs Alter IdempotentWrite Read Delete Create ClusterAction All Write AlterConfigs (default: All)')
...
_kafka_avro_console_consumer_args+=('--isolation-level: Set to read_committed in order to filter out transactional messages which are not committed. Set to read_uncommitted to read all messages. (default: read_uncommitted)')
...
_kafka_avro_console_producer_args+=('--bootstrap-server: REQUIRED unless --broker-list (deprecated) is specified. The server (s) to connect to. The broker list string in the form HOST1:PORT1,HOST2: PORT2.')
_kafka_avro_console_producer_args+=('--broker-list: DEPRECATED, use --bootstrap-server instead; ignored if --bootstrap- server is specified. The broker list string in the form HOST1:PORT1, HOST2:PORT2.')
...
_kafka_avro_console_producer_args+=('--property: A mechanism to pass user-defined properties in the form key=value to the message reader. This allows custom configuration for a user- defined message reader. Default properties include: parse.key=true|false key.separator=<key.separator> ignore.error=true|false')
...
_kafka_configs_args+=('--add-config-file: Path to a properties file with configs to add. See add-config for a list of valid configurations.')
_kafka_configs_args+=('--all: List all configs for the given entity (includes static configuration when the entity type is brokers)')
...
_kafka_configs_args+=('--broker: The broker''s ID.')
_kafka_configs_args+=('--broker-defaults: The config defaults for all brokers.')
_kafka_configs_args+=('--broker-logger: The broker''s ID for its logger config.')
_kafka_configs_args+=('--client: The client''s ID.')
_kafka_configs_args+=('--client-defaults: The config defaults for all clients.')
_kafka_configs_args+=('--cluster-link: The cluster link''s name.')
...
_kafka_configs_args+=('--entity-name: Name of entity (topic name/client id/user principal name/broker id/cluster link)')
_kafka_configs_args+=('--entity-type: Type of entity (topics/clients/users/brokers/broker- loggers/cluster-links)')
...
_kafka_console_consumer_args+=('--isolation-level: Set to read_committed in order to filter out transactional messages which are not committed. Set to read_uncommitted to read all messages. (default: read_uncommitted)')
...
_kafka_console_producer_args+=('--bootstrap-server: REQUIRED unless --broker-list (deprecated) is specified. The server (s) to connect to. The broker list string in the form HOST1:PORT1,HOST2: PORT2.')
_kafka_console_producer_args+=('--broker-list: DEPRECATED, use --bootstrap-server instead; ignored if --bootstrap- server is specified. The broker list string in the form HOST1:PORT1, HOST2:PORT2.')
...
_kafka_console_producer_args+=('--property: A mechanism to pass user-defined properties in the form key=value to the message reader. This allows custom configuration for a user- defined message reader. Default properties include: parse.key=true|false key.separator=<key.separator> ignore.error=true|false')
...
_kafka_consumer_perf_test_args+=('--bootstrap-server: REQUIRED unless --broker-list (deprecated) is specified. The server (s) to connect to.')
_kafka_consumer_perf_test_args+=('--broker-list: DEPRECATED, use --bootstrap-server instead; ignored if --bootstrap- server is specified. The broker list string in the form HOST1:PORT1, HOST2:PORT2.')
...
_kafka_consumer_perf_test_args+=('--group: The group id to consume on. (default: perf-consumer-20221)')
...
_kafka_topics_args+=('--invalid-replica-placement-partitions: if set when describing topics, only show partitions whose placement doesn''t adhere to the replica placement constraints. Not supported with the --zookeeper option.')
_kafka_topics_args+=('--link-name: The cluster link name for topic mirroring. Requires --bootstrap- server option.')
_kafka_topics_args+=('--list: List all available topics.')
_kafka_topics_args+=('--mirror-action: The action to perform when altering a topic''s mirror, supported: {stop}.')
_kafka_topics_args+=('--mirror-topic: The topic that will be mirrored over the cluster link. Requires -- bootstrap-server and --link-name options.')
...
_kafka_streams_application_reset_args+=('--intermediate-topics: Comma-separated list of intermediate user topics (topics that are input and output topics, e.g., used in the deprecated through() method). For these topics, the tool will skip to the end.')
...
_kafka_reassign_partitions_args+=('--additional: Execute this reassignment in addition to any other ongoing ones.')
_kafka_reassign_partitions_args+=('--bootstrap-server: the server(s) to use for bootstrapping. REQUIRED if an absolute path of the log directory is specified for any replica in the reassignment json file, or if -- zookeeper is not given.')
...
_kafka_reassign_partitions_args+=('--timeout: The maximum time in ms to wait for log directory replica assignment to begin. (default: 10000)')
...
_kafka_reassign_partitions_args+=('--zookeeper: DEPRECATED: The connection string for the zookeeper connection in the form host:port. Multiple URLS can be given to allow fail-over. Please use --bootstrap-server instead.')
